def read_map
  ...
  File.readlines('input.txt', chomp: true).each_with_index do |line, index|
    ...
    start_position_index = line.index('E')
...
def find_shortest_path(heightmap, current_position, visited_positions, moves)
  ...
  return moves if heightmap[current_position[:y]][current_position[:x]] == 'a'
  ...
  possible_next_positions.each do |next_position|
    ...
    next if (height_at(heightmap, current_position) - height_at(heightmap, next_position)) >= 2
