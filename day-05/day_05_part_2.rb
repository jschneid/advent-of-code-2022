...
def perform_crate_move(crate_quantity, from_position, to_position, stacks)
  moved_crates = stacks[from_position].pop(crate_quantity)
  stacks[to_position].push(*moved_crates)
end

def perform_crate_move_line(line, stacks)
  tokenized_line = line.split(' ')
  crate_quantity = tokenized_line[1].to_i
  from_position = tokenized_line[3].to_i - 1
  to_position = tokenized_line[5].to_i - 1
  perform_crate_move(crate_quantity, from_position, to_position, stacks)
end
