...
def solve(possible_expedition_positions, map_states, steps, exit)
  loop do
...
entrance = Point.new(x: 0, y: 0)

trip_1_steps = solve(Set[Point.new(x: 0, y: -1)], map_states, 0, exit)
trip_2_steps = solve(Set[Point.new(x: @field_width - 1, y: @field_height - 1)], map_states, trip_1_steps, entrance)
p solve(Set[Point.new(x: 0, y: -1)], map_states, trip_2_steps, exit)
